...
module GtkDriver
  ...
  class Session
      ...
      Gsk::CairoRenderer.new.tap do |renderer|
        ...
        begin
          Gtk::Snapshot.new.then do |snapshot|
            paintable_for(widget).snapshot(snapshot, widget.width, widget.height)
    ...
    # A GtkWidgetPaintable only starts observing its widget on the next frame,
    # so a freshly built one snapshots to nothing and the shot fails at random.
    # Keeping one per widget for the whole session means it is always warm.
    def paintable_for(widget)
      @paintables ||= {}
      @paintables[widget.object_id] ||= Gtk::WidgetPaintable.new(widget)
    end
